...
public class KarateFeatureGenerator {
    ...
    private static void updateFeatureFile(File featureFile, OpenAPI openAPI, String path, String method, Operation operation) {
        try {
            ...
            Set<String> swaggerParams = new HashSet<>();
            Set<String> swaggerRequestBodyKeys = new HashSet<>();
            Map<String, String> swaggerKeyTypes = new HashMap<>(); // Store types (query, header, requestBody)
            ...
            Set<String> existingRequestBodyKeys = new HashSet<>();
            boolean inRequestBody = false;

            for (String line : existingContent) {
                if (line.contains("* param")) {
                    existingParams.add(extractKeyFromLine(line));
                } else if (line.contains("request {")) {
                    inRequestBody = true;
                } else if (inRequestBody && line.contains("}")) {
                    inRequestBody = false;
                } else if (inRequestBody) {
                    existingRequestBodyKeys.add(extractKeyFromLine(line));
                }
            }

            // Detect removed and added keys
            Set<String> removedParams = new HashSet<>(existingParams);
            removedParams.removeAll(swaggerParams);

            Set<String> addedParams = new HashSet<>(swaggerParams);
            addedParams.removeAll(existingParams);

            Set<String> removedRequestBodyKeys = new HashSet<>(existingRequestBodyKeys);
            removedRequestBodyKeys.removeAll(swaggerRequestBodyKeys);

            Set<String> addedRequestBodyKeys = new HashSet<>(swaggerRequestBodyKeys);
            addedRequestBodyKeys.removeAll(existingRequestBodyKeys);

            // Generate updated content
            boolean requestBodyModified = false;
            for (String line : existingContent) {
                String key = extractKeyFromLine(line);

                if (removedParams.contains(key)) {
                    updatedContent.add(line + " // removed");
                } else if (removedRequestBodyKeys.contains(key)) {
                    updatedContent.add(line + " // removed");
                } else {
                    updatedContent.add(line);
                }

                // Add new request body keys at the right position
                if (line.contains("request {") && !requestBodyModified) {
                    requestBodyModified = true;
                    for (String addedKey : addedRequestBodyKeys) {
                        updatedContent.add("    \"" + addedKey + "\": \"<value>\", // added");
                    }
                }
            }
